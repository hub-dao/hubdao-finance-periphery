...
import HubdaoFactory from '@hubdao-finance/hubdao-core/build/HubdaoFactory.json'
...
import HubdaoRouter from '../../build/HubdaoRouter.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy routers
  const router01 = await deployContract(wallet, HubdaoRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, HubdaoRouter, [factoryV2.address, WETH.address], overrides)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IHubdaoPair.abi), provider).connect(wallet)
