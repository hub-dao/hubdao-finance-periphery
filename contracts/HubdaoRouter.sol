...
import '@hubdao-finance/hubdao-core/contracts/interfaces/IHubdaoFactory.sol';
...
contract HubdaoRouter is IHubdaoRouter02 {
    ...
    function addLiquidityETH(
        ...
        address pair = HubdaoLibrary.pairFor(factory, token, WETH);
    ...
    {
        require(path[0] == WETH, 'HubdaoRouter: INVALID_PATH');
        amounts = HubdaoLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'HubdaoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(HubdaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'HubdaoRouter: INVALID_PATH');
        amounts = HubdaoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'HubdaoRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HubdaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(HubdaoLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
